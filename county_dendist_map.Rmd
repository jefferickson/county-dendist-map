...
county.city.list.shortest$rel.missoula <- apply(county.city.list.shortest, 1, function(x) {
  if (x["county.geo.id"] == missoula.fips) {
    return(0)
  } else if (x["den.dist.pop.index"] < county.city.list.shortest$den.dist.pop.index[which(county.city.list.shortest$county.geo.id == missoula.fips)]) {
    return(-1)
  } else if (x["den.dist.pop.index"] > county.city.list.shortest$den.dist.pop.index[which(county.city.list.shortest$county.geo.id == missoula.fips)]) {
    return(1)
  } else {
